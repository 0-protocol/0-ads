...
impl AttentionOracle {
    pub async fn verify_github_star(&self, agent_github_id: &str, target_repo: &str) -> Result<Vec<u8>, VMError> {
        ...
        let resp = self.client.get(&url).send().await.map_err(|_| VMError::ExecutionError("Oracle fetch failed".into()))?;
        ...
        if resp.status().is_success() {
            ...
            Err(VMError::ExecutionError("Agent did not star repo".into()))
        }
    }
